# План работ: Login + Todo List

Стек: **Vue 3 + Vue Router + Tailwind CSS**

## 0. Уточнение по ТЗ

- Использовать **Vue 3**.
- Использовать **Vue Router** для перехода между страницами.
- Использовать **Tailwind CSS** для адаптивной вёрстки.
- Данные брать из REST API:
  - users: `https://jsonplaceholder.typicode.com/users`
  - todos: `https://jsonplaceholder.typicode.com/todos`
- Текущего пользователя хранить в памяти приложения через `provide/inject`.
- Избранные todo хранить в `localStorage`.
- Не хранить в `localStorage` данные авторизации, фильтры, поиск и созданные todo, потому что этого нет в ТЗ.

---

## 1. Создание проекта

- [x] Создать Vue-проект через Vite и перейти в папку проекта:
- [x] Инициализировать Git-репозиторий (`git init`).
- [x] Создать первый коммит после базовой настройки проекта.
- [x] Установить Vue Router
- [x] Установить Tailwind CSS:
- [x] Подключить Tailwind в `vite.config.ts`.
- [x] Подключить Tailwind в главный CSS-файл `src/style.css`.
- [x] Проверить запуск проекта:

---

## 2. Базовая структура проекта

- [x] Привести проект к понятной структуре:

```text
src/
  api/
    jsonPlaceholderApi.ts
  router/
    index.ts
  pages/
    LoginPage.vue
    TodosPage.vue
  components/
    LoginForm.vue
    UserInfo.vue
    TodoFilters.vue
    TodoCreateForm.vue
    TodoList.vue
    TodoItem.vue
  types/
    user.ts
    todo.ts
  composables/
    authContext.ts
  utils/
    localStorage.ts
  App.vue
  main.ts
  style.css
```

- [x] Вынести типы `User` и `Todo` в отдельные файлы.
- [x] Вынести работу с API в отдельный файл.
- [x] Вынести auth state текущего пользователя в `provide/inject`.
- [x] Вынести работу с `localStorage` в отдельный utility-файл только для избранных todo.

---

## 3. Типы данных

### `User`

- [x] Создать тип пользователя на основе ответа API:

### `Todo`

- [x] Создать тип задачи:

---

## 4. Настройка Vue Router

- [x] Создать роутер в `src/router/index.ts`.
- [x] Добавить два маршрута:
- [x] Подключить роутер в `main.ts`.
- [x] В `App.vue` оставить только `<RouterView />`.
- [x] Проверить переход между страницами вручную.

---

## 5. API-слой

- [x] Создать файл `src/api/jsonPlaceholderApi.ts`.
- [x] Реализовать функцию получения пользователей:
- [x] Реализовать функцию получения todo:
- [x] Реализовать функцию создания todo:

---

## 6. Страница входа

### Основная задача

- [x] На главной странице `/` вывести форму входа.
- [x] В форме должны быть поля:
  - `Username`
  - `Phone number`
  - кнопка `Login`

### Валидация username

- [x] Поле `Username` обязательное.
- [x] Поле `Username` должно принимать только буквы и символ "." (точка). Необходимость принимать точку не соответствует ТЗ, однако она вызвана тем, что в списке пользователей есть `usernae`, которые содержат точку.

### Валидация phone

- [x] Поле `Phone number` обязательное.
- [x] Поле `Phone number` должно принимать цифры и символы +, (, ), -, пробел.

### Логика входа

- [x] При нажатии `Login` загрузить список пользователей.
- [x] Найти пользователя, у которого совпадают:
  - `username`
  - `phone`
- [x] Если пользователь найден:
  - записать пользователя в auth context через `provide/inject`;
  - выполнить редирект на `/todos`.
- [x] Если пользователь не найден:
  - показать ошибку `Username or Phone Number does not match`.

### Хранение текущего пользователя через provide/inject

- [x] Создать auth context, который хранит пользователя в памяти приложения.
- [x] В auth context описать:
  - `currentUser` — текущий пользователь;
  - `setCurrentUser(user)` — запись пользователя после успешного login;
  - `clearCurrentUser()` — очистка пользователя при logout.
- [x] Вызвать `provideAuth()` в верхнем компоненте, например в `App.vue`.
- [x] После успешного login вызвать `setCurrentUser(user)`.
- [x] На странице `/todos` читать пользователя через `useAuth()`.

---

## 7. Защита страницы todos

- [x] В `TodosPage.vue` получить `currentUser` через `useAuth()`.
- [x] Если пользователя нет в auth context, внутри `onMounted()` выполнить редирект на `/`.
- [x] Важно: после перезагрузки страницы auth context очистится, поэтому пользователь вернётся на login. Это нормально, потому что ТЗ не требует хранить сессию.
- [x] Более аккуратный вариант — добавить navigation guard в Vue Router, но для тестового задания достаточно проверки в компоненте.

---

## 8. Страница todos

- [x] Создать компонент `TodoList.vue`.

### Данные пользователя

- [x] В верхней части страницы вывести персональные данные пользователя:
  - name
  - username
  - email
  - phone
  - website
  - company name
  - city
- [x] Оформить блок пользователя карточкой через Tailwind.

### Загрузка todos

- [x] При открытии страницы `/todos` загрузить todos из API.
- [x] Сохранить их в реактивный массив:
- [x] Добавить состояние загрузки:
- [x] Добавить состояние ошибки:
- [x] При ошибке показать сообщение на странице.

---

## 9. Вывод списка todo

- [x] Создать компонент `TodoItem.vue`.
- [x] Передавать todo в `TodoItem` через props.
- [x] Вывести для каждой задачи:
  - id
  - userId
  - title
  - completed/uncompleted
  - кнопку или иконку избранного
- [x] Использовать `v-for`:
- [x] Для completed/uncompleted использовать разные иконки.

---

## 10. Фильтр по статусу

- [x] Создать select со статусами:
  - `All`
  - `Completed`
  - `Uncompleted`
  - `Favorites`
- [x] Создать реактивное состояние:
- [x] В computed-фильтрации реализовать правила:
  - `all` — показывать все задачи;
  - `completed` — только `todo.completed === true`;
  - `uncompleted` — только `todo.completed === false`;
  - `favorites` — только задачи, id которых есть в избранном.

---

## 11. Фильтр по userId

- [x] Создать select с опциями:
  - `All Users`
  - уникальные `userId` из списка todos
- [x] Получить уникальные userId через computed:
- [x] Создать состояние выбранного пользователя:
- [x] Если выбран `All Users`, не фильтровать по пользователю.
- [x] Если выбран конкретный userId, показывать только его задачи.

---

## 12. Поиск по title

- [x] Добавить input для поиска.
- [x] Создать состояние для поиска:
- [x] Поиск должен работать вместе с выбранными фильтрами.
- [x] Сравнение делать без учёта регистра:

## 13. Общая computed-фильтрация

- [x] Создать `filteredTodos` через `computed`.
- [x] Последовательно применить:
  - фильтр по статусу;
  - фильтр по userId;
  - поиск по title.

---

## 14. Блок Create todo

Ответственность разделена между `create-todo.vue` (сам запрос) и `todos-list.vue`
(что делать со списком после успешного создания).

### `create-todo.vue`

- [x] Внутри компонента добавить поля:
  - поле `Title`
  - кнопка `Add`
- [x] `Title` не должен быть пустым.
- [x] `Title` не должен быть более 255 символов.
- [x] Поле `userId` не добавлено сознательно, вразрез с ТЗ. Дискуссионный вопрос. Решение принято самостоятельно ввиду того, что задание тестовое
- [x] `userId` берем `currentUser.id` через `useAuth()`.
- [x] При клике `Add` вызвать `createTodo()` из api-слоя (POST `/todos`).
- [x] Показывать состояние загрузки (`isSubmitting`) и блокировать поле и кнопку на время запроса.
- [x] При ошибке создания показывать сообщение об ошибке в этом же компоненте.
- [x] Очищать поле `Title` только после успешного ответа API.
- [x] После успешного создания эмитить `created` с созданной задачей (ответ `createTodo()`).

### `todos-list.vue`

- [x] Слушать событие `created` от `CreateTodo`.
- [x] По этому событию перезапросить список через `getTodos()`.
- [x] Так как jsonplaceholder не сохраняет POST-данные, перезапрошенный список не будет
      содержать созданную задачу — добавляем её в начало списка вручную из полученного в
      событии `created` объекта (`todos.value = [newTodo, ...await getTodos()]`, одним
      присваиванием).
- [x] Обработку `created` сериализовать через `createQueue` (serial promise chain), чтобы
      исключить гонку при двух последовательных созданиях подряд:
  - `createQueue` — промис-"хвост" цепочки, изначально `Promise.resolve()`.
  - Каждый вызов `onTodoCreated` пристраивает свою задачу в конец текущей цепочки и сохраняя новый хвост.
  - Если предыдущая задача ещё не завершилась (например, второй `created` прилетел, пока
    первый ещё внутри `await getTodos()`), новый колбэк физически не может стартовать раньше —
    он выполнится только после того, как резолвится текущий `createQueue`.
  - В результате несколько подряд идущих созданий гарантированно применяются к `todos.value`
    строго в порядке поступления, а не гонятся друг с другом за то, чей результат победит.
  - Важно: это не устраняет тот факт, что jsonplaceholder не сохраняет POST-данные — при создании
    следующего todo предыдущий всё ещё будет вытеснен из списка после рефетча (см. известное
    ограничение выше), сериализация делает это поведение упорядоченным.

---

## 15. Избранное через localStorage

- [x] Создать ключ для localStorage:
- [x] Хранить только массив id:
- [x] При загрузке страницы читать избранное из `localStorage`.
- [x] Если данных нет — использовать пустой массив.
- [x] Реализовать функцию добавления/удаления из избранного:
- [x] Передавать в `TodoItem`:
  - является ли задача избранной;
  - обработчик переключения избранного.
- [x] Для избранной задачи показать активное состояние кнопки `★`.
- [x] Для обычной задачи показать `☆`.

---

## 20. Проверка сценариев

### Login success

- [x] Открыть `/`.
- [x] Ввести существующий username из API.
- [x] Ввести соответствующий phone.
- [x] Нажать `Login`.
- [x] Проверить редирект на `/todos`.
- [x] Проверить, что данные пользователя отображаются сверху.

### Login error

- [x] Ввести неправильный username или phone.
- [x] Нажать `Login`.
- [x] Проверить сообщение `login error`.
- [x] Убедиться, что редиректа нет.

### Todos

- [x] Проверить, что все todos загрузились.
- [x] Проверить фильтр `Completed`.
- [x] Проверить фильтр `Uncompleted`.
- [x] Проверить фильтр `Favorites`.
- [x] Проверить фильтр по userId.
- [x] Проверить поиск по title.
- [x] Проверить совместную работу всех фильтров.

### Create todo

- [x] Ввести Title.
- [x] Нажать Add.
- [x] Проверить, что todo появился в списке.

### Favorites

- [x] Добавить todo в избранное.
- [x] Обновить страницу.
- [x] Проверить, что избранное сохранилось.
- [x] Выбрать фильтр `Favorites`.
- [x] Проверить, что отображаются только избранные задачи.

---

## 22. Критерии готовности

- [x] Проект запускается через `npm run dev`.
- [x] Главная страница содержит форму login.
- [x] Username принимает только буквы.
- [x] Phone принимает числа и символы.
- [x] Login success работает по данным users API.
- [x] Login error отображается при несовпадении данных.
- [x] После успешного входа открывается страница todos.
- [x] На странице todos отображаются данные пользователя.
- [x] Todos загружаются из API и выводятся списком.
- [x] Работает фильтр по статусу.
- [x] Работает фильтр по userId.
- [x] Работает поиск по title.
- [x] Работает создание todo.
- [x] Работает добавление todo в избранное.
- [x] Избранное сохраняется в localStorage.
- [x] Работает фильтр Favorites.
- [x] Интерфейс адаптивный.
- [x] Код разделён на страницы, компоненты, api, types и utils.
